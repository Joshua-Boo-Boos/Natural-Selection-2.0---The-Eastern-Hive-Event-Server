...
local function PerformAttackEntity( eyePos, bestTarget, bot, brain, move )
    ...
    if distance < 4 then
        ...
        if brain.lastIsStomping and brain.lastIsStomping ~= isStomping then
            --brain.wantsToStomp = false --Orginal = false (fehler der mit der zeit bewirkt das der onos nicht mehr k�mpfen kann und in seiner stomp animation festh�ngt, wenn auf false) bot kann sich selbst debuggen wenn der fehler autritt (SwitchToBoneShield) /false bezieht sich nur auf den code if not brain.wantsToStomp wenn er aktiv ist.  
        end
