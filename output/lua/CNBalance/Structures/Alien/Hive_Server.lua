...
function Hive:GetTimeLastARCDamageReceived()
    return self.timeLastReceivedARCDamage
end
...
local function UpdateHealing(self)
    ...
    if GetIsUnitActive(self) and not self:GetGameEffectMask(kGameEffect.OnFire) and not self:GetElectrified() then
...
function Hive:GenerateEggSpawns(hiveLocationName)
    ...
    if #self.eggSpawnPoints < kAlienEggsPerHive * 2 then
        Print("Hive in location \"%s\" only generated %d egg spawns (needs %d). Place some egg entities.", hiveLocationName, table.icount(self.eggSpawnPoints), kAlienEggsPerHive)
    end
...
function Hive:OnTakeDamage(damage, attacker, doer, point)
    ...
    if damage > 0 then
        ...
        if doer and doer:GetClassName() == "ARC" then
            self.timeLastReceivedARCDamage = time
        end
